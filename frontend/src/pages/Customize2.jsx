...
function Customize2() {
    ...
    return (
        ...
        <input type="type" placeholder='eg: sifra' className='w-full max-w-[600px] h-[60px] outline-none border-2 border-white bg-transparent text-white placeholder-gray-300 px-[20px] py-[10px] rounded-full text-[18px]' 
        required onChange={(e)=>setAssistantName(e.target.value)} value={assistantName} />

           {assistantName &&
            <button className='mt-[30px] min-w-[300px] h-[60px] bg-white text-blue rounded-full text- [18px] font-semibold hover:bg-blue-600 transition duration-300'
            disabled={loading} 
            onClick={()=>{
        
                handleUpdateAssistantName();
            }
            }> {!loading?"Finish Customization ":"Loading.."}</button>}
        </div>
    );
}
